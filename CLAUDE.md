...
- `--judge-diagnosis` — also score each diagnosis's groundedness + completeness + actionability with the LLM judge (needs `--failure-analysis`; adds one judge call per failed test). CLI equivalent of `AI_JUDGE_ENABLED=true`, which is how Docker/K8s/CI enable it (in GitHub Actions via the `AI_JUDGE_ENABLED` repo variable) — mirroring the `--failure-analysis` ↔ `AI_ANALYSIS_ENABLED` pair. Enabling judging also turns on an **agentic refine loop**: the judge critiques the diagnosis and the analyzer improves it (feeding the judge's failed-dimension issues back in) until the judge passes and confidence ≥ `AI_REFINE_CONFIDENCE_TARGET` (default 90), capped at `AI_REFINE_MAX_ITERS` passes (default 2)
...
- Dedicated per-client fixtures — `movies_api`, `people_api`, `lists_api`, `search_api`, `discover_api`, `networks_api`, `trending_api`, `reviews_api` — each returns a fresh, type-annotated endpoint client (e.g. `MoviesAPI()`); a test declares the one it needs in its signature. (`AccountAPI` has no fixture; it's used directly in `tests/helpers/test_data_generators.py`.)
...
- Hooks: when `--failure-analysis` is on, `pytest_runtest_makereport` diagnoses each failure and attaches it to Allure; adding `--judge-diagnosis` (or `AI_JUDGE_ENABLED=true`) also scores each diagnosis via `evals.judge.judge_live` (judge model overridable with `AI_JUDGE_MODEL`) and drives the agentic refine loop (`failure_analyzer.refine_until_confident`: judge → `analyzer.refine` → re-judge, bounded by `AI_REFINE_MAX_ITERS` / `AI_REFINE_CONFIDENCE_TARGET`), so the *final* refined diagnosis is what gets logged, attached, and saved. The judge is injected into the loop, keeping `tests/helpers` decoupled from `evals`. `pytest_sessionfinish` handles Allure/HTML report customization and `environment.properties`, and appends one run-level token/cost+quality row to `ai_analysis/token_usage.jsonl` (see `telemetry.py` below). `pytest_terminal_summary` re-prints all accumulated diagnoses as an end-of-run console section (also fires under `AI_ANALYSIS_ENABLED=true`; silent when nothing was analyzed) plus a one-line token/cost recap.
...
**`tests/journeys/`** (allure feature `Journeys`) — multi-endpoint integration tests that chain live calls the way `JourneyUser` does in perf: one test declares several client fixtures, drills an id out of one response, feeds it into the next, and asserts the chain resolves. `test_search_to_details.py` searches `search/movie` then opens `movie/{id}` for the top result; `test_trending_to_review.py` is a 3-hop chain — list `trending/movie/{window}`, open the first trending movie's `movie/{id}/reviews`, then drill into `review/{review_id}`, asserting the review's `media_id` points back to that movie (and it's the only test that schema-validates the `movie/{id}/reviews` list body, via `movie_reviews_schema`). Each step still validates with the same `assert_get_metadata` + `load_schema` helpers; seed/metadata data is reused from existing `test_data.yaml` sections, not a new YAML block. No marker — these run in the default live suite (and hit ≥2 endpoints, so are marginally flakier); a journey may `pytest.skip` when live data can't satisfy the chain (e.g. no trending movie has reviews).
...
**`telemetry.py`** — a process-wide `TokenLedger` singleton (`ledger`) at repo root that salvages the Groq `usage` block (prompt/completion tokens) both the analyzer (`_request_diagnosis`) and judge (`_call_judge`) would otherwise discard; each records one call via `ledger.record(model, role, usage, test_name)`. It lives at repo root — neutral ground — so `evals` doesn't import from `tests/`. Recording is defensive (a malformed/`None` `usage` counts as zero, never raises). `estimate_cost_usd` derives a best-effort dollar figure from a small dated per-model price table (`_PRICE_PER_MILLION`; unlisted models count tokens but $0). At session end `pytest_sessionfinish` appends **one run-level row** — `{ts, git_sha, analyzer_model, judge_model, judged, n_failures, calls, prompt/completion/total tokens, est_cost_usd, judge_pass_rate, mean_confidence}` — to `ai_analysis/token_usage.jsonl` (gitignored), deliberately co-locating cost with quality so both can be trended across runs. Local-only for now; CI-runner persistence is a follow-up.
...
- When adding a new Pydantic schema, register it in **both** `tests/schemas/models.py` and the `load_schema` map in `conftest.py`. The paginated movie-list responses (`PopularMoviesResponse`, `SearchMoviesResponse`, `DiscoverMoviesResponse`, `TrendingMoviesResponse`) don't repeat the `page`/`results`/`total_pages`/`total_results` envelope — they subclass the `paginated_movie_list(ge=1)` factory base (popular passes `ge=0` for its looser bounds), keeping a distinct registered name per endpoint. `SearchMoviesResponse` additionally redeclares `results` as `list[SearchMovieListItem]` — a `MovieListItem` subclass that makes `genre_ids`/`popularity`/`release_date`/`video`/`vote_average`/`vote_count` optional (same strict types/bounds when present) because TMDB search can return stub catalog entries that omit those keys entirely. Pydantic does **not** merge a subclass's fields into a nested model, so declaring such overrides on the envelope class does nothing; the `results` annotation itself must be replaced. A new list-style endpoint should subclass it too — the exception is `MovieReviewsResponse` (`movie/{id}/reviews`), whose `results` are reviews (`ReviewListItem`), not movies, and whose envelope also carries a top-level `id` (the movie id), so it's a standalone model rather than a factory subclass.
...
- Most `$placeholder` generators are cheap (in-memory/disk), but `$random_review_id` (`pick_random_review_id`) makes **live API calls at collection time** — it picks random movie ids until one has reviews (most movies have none), so the review test module needs network access just to collect. Bounded by `max_attempts`.
- A review's `author_details.avatar_path` (`ReviewDetails`/`AuthorDetails`) can be a gravatar URL (`/https://secure.gravatar.com/...`), **not** an image path — so unlike other `*_path` fields it must **not** carry the `.png|.jpg` regex.
