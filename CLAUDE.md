...
- When adding a new Pydantic schema, register it in **both** `tests/schemas/models.py` and the `load_schema` map in `conftest.py`. The paginated movie-list responses (`PopularMoviesResponse`, `SearchMoviesResponse`, `DiscoverMoviesResponse`, `TrendingMoviesResponse`) don't repeat the `page`/`results`/`total_pages`/`total_results` envelope — they subclass the `paginated_movie_list(ge=1)` factory base (popular passes `ge=0` for its looser bounds), keeping a distinct registered name per endpoint. A new list-style endpoint should subclass it too — the exception is `MovieReviewsResponse` (`movie/{id}/reviews`), whose `results` are reviews (`ReviewListItem`), not movies, and whose envelope also carries a top-level `id` (the movie id), so it's a standalone model rather than a factory subclass.
